...
class Cityscapes(data.Dataset):
    def __init__(
        self,
        root,
        list_path="./list/cityscapes/train.txt",
        max_iters=None,
        crop_size=(321, 321),
        mean=(128, 128, 128),
        vars=(1, 1, 1),
        ...
        for item in self.img_ids:
            ...
            img_file = osp.join(self.root + "/leftImg8bit_trainvaltest", image_path)
            label_file = osp.join(self.root + "/gtFine_trainvaltest", label_path)
    ...
    def generate_scale_label(self, image, label):
        ...
        image = cv2.resize(
            image, None, fx=f_scale, fy=f_scale, interpolation=cv2.INTER_LINEAR
        )
        label = cv2.resize(
            label, None, fx=f_scale, fy=f_scale, interpolation=cv2.INTER_NEAREST
        )
        return image, label
    ...
    def __getitem__(self, index):
        ...
        label = torch.from_numpy(label.astype(np.long)).clone()
